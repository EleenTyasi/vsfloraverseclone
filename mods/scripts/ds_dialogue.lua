--------------------------------------------------------------------------------
-- DS Dialogue System for Psych Lua                          ©DragShot - v1.1.0
--------------------------------------------------------------------------------
...
function textTyper.update(self, elapsed)
	...
	local utf8len = utf8.len(self.text);
...
function readDialogueLines(name)
	...
	local targets = {
		'data/'..songPath..'/'..name..langSufx..'.txt',
		'data/'..songPath..'/'..name..'.txt',
		'data/'..songName..'/'..name..langSufx..'.txt',
		'data/'..songName..'/'..name..'.txt',
		'data/'..name..langSufx..'.txt',
		'data/'..name..'.txt'
	};
	local textData;
...
function diag.endDialogue(self, notify)
	...
	if (self.onDisplay.diagName) then
		soundFadeOut('sfx:voiceOver', 0.1);
		diag:clearBg(false);
		textTyper:reset();
